...
class OutputFormatter:
    ...
    def format_telegram_message(
        ...
        # Header
        message_parts.append("🎬 *MASTER PROMPT (Text-to-Video)*")
        message_parts.append(f'`{prompt_data.get("master_prompt", "N/A")}`')
        message_parts.append("")

        # Visual Specifications
        if "visual_specifications" in prompt_data:
            message_parts.append("📊 *VISUAL SPECIFICATIONS*")
        ...
        # Platform Tips
        if platform_specifics:
            message_parts.append("📱 *PLATFORM TIPS*")

            # Show top 2 platforms
            for i, (platform, specs) in enumerate(list(platform_specifics.items())[:2]):
                message_parts.append(f"\n*{platform.capitalize()}*:")
                message_parts.append(f"• Ratio: {specs['aspect_ratio']}")
                message_parts.append(f"• Length: {specs['optimal_length']}")
        ...
        if "hashtags" in prompt_data:
            ...
            hashtag_string = " ".join(f"#{tag}" for tag in hashtags)
            message_parts.append(f"🔖 {hashtag_string}")
            message_parts.append("")

        # Footer
        message_parts.append("\n✨ Generated by RekaKata")
